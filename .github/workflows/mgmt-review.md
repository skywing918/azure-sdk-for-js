...
- If `Check-format FAILED` is detected, add label `auto-format-needed` via `add-labels`. A separate trusted workflow will checkout the PR code and run `pnpm format` safely without access to repository secrets.
